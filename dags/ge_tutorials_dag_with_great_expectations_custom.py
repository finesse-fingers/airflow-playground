...
great_expectations_context_path = os.getenv('GE_TUTORIAL_GE_CONTEXT_PATH') or os.path.join(
...
# The DAG definition
dag = DAG(
    dag_id='ge_tutorials_dag_with_ge_custom',
    default_args=default_args,
    schedule_interval=None,
    tags=["bobby", "great_expectations", "ge", "custom"],
)
...
def load_files_into_db(ds, **kwargs):
    ...
    with engine.connect() as conn:
        ...
        df_npi_small = pd.read_csv(os.path.join(
            GE_TUTORIAL_ROOT_PATH, "data", "npi_small.csv"))
        column_rename_dict = {old_column_name: old_column_name.lower(
        ) for old_column_name in df_npi_small.columns}
        ...
        df_state_abbreviations = pd.read_csv(os.path.join(
            GE_TUTORIAL_ROOT_PATH, "data", "state_abbreviations.csv"))
...
def validate_source_data(ds, **kwargs):
    ...
    batch_file = context.get_batch(
        batch_kwargs_file, 'npi_small_file.critical')
...
def validate_source_data_load(ds, **kwargs):
    ...
    batch_kwargs_file = {"path": os.path.join(GE_TUTORIAL_ROOT_PATH, "data", "npi_small.csv"),
                         'datasource': 'input_files'}
    batch_file = context.get_batch(
        batch_kwargs_file, expectation_suite_name_file)
    ...
    batch_kwargs_db = {'table': "npi_small",
    ...
    run_id = "airflow:" + kwargs['dag_run'].run_id + \
        ":" + str(kwargs['dag_run'].start_date)
    results = context.run_validation_operator(
        "action_list_operator",
    ...
    if not results["success"]:
        raise AirflowException(
            "Validation of the source data loading is not successful ")
...
def validate_analytical_output(ds, **kwargs):
    ...
    # a datasource configured in your great_expectations.yml
    datasource_name = "datawarehouse"
    ...
    batch_kwargs = {"table": "count_providers_by_state",
    ...
    if not results["success"]:
        raise AirflowException(
            "The analytical output does not meet the expectations in the suite: {0:s}".format(expectation_suite_name))
...
def ge_validate_source_data(ds, **kwargs):
    context = ge.data_context.DataContext(great_expectations_context_path)
    ...
    batch_file = context.get_batch(
        batch_kwargs_file, 'npi_small_file.custom')
